      ...
      try {
         ...
         if (style == 1) {
            if (text) {
               ...
               await client.sendList(m.chat, '', global.db.setting.msg.replace('+tag', `@${m.sender.replace(/@.+/g, '')}`).replace('+name', m.pushName).replace('+greeting', Func.greeting()), global.botname, 'SENTUH AKU >//<!', [{
                  rows
               }], m)
            }
         } else if (style == 2) {
