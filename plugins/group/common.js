...
exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         if (arr.length == 0) return client.reply(m.chat, Func.texted('bold', `gada grup yang sama bot`), m)
         arr.map(x => {
         ...
         client.sendList(m.chat, '', `Bot dan @${user.replace(/@.+/,'')} ada di *${arr.length}* groups.`, '', 'Klik disini ea!', [{
            rows
         }], m)
      }
   },
