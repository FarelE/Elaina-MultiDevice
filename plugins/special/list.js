...
exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         if (!args || !args[0]) return client.sendList(m.chat, '', 'Pilih data yang kamu mau', '', 'Klik disini ea!', [{
            rows
         }], m)
         if (args[0] == 1) {
            const data = global.db.users.filter(v => v.banned)
            if (data.length == 0) return client.reply(m.chat, Func.texted('bold', `Tidak ada data.`), m)
