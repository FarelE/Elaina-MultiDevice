exports.run = {
   ...
   async: async (m, {
      ...
      if (await client.onWhatsApp(number).length == 0) return client.reply(m.chat, Func.texted('bold', `Nomor tidak terdaftar WhatsApp.`), m)
      try {
         let mention = number.replace(/@.+/g, '')
         let users = m.isGroup ? participants.find(u => u.id == number) : {}
         if (!users) return client.reply(m.chat, Func.texted('bold', `@${mention} udah keluar dari grup / gaada orangnya.`), m)
