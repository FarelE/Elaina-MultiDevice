   ...
   async: async (m, {
      ...
      try {
         if (command == 'apk') {
            ...
            client.sendList(m.chat, '', `Menampilkan hasil pencarian dari : “${text}”, pilih aplikasi dibawah ini`, '', 'Klik disini ea!', [{
               rows
            }], m)
            ...
            if (chSize.oversize) return client.reply(m.chat, `File size (${json.file.size}) exceeds the maximum limit, download sendiri di : ${await (await scrap.shorten(json.file.url)).data.url}`, m)
