   ...
   async: async (m, {
      ...
      try {
         ...
         client.reply(m.chat, Func.texted('bold', global.status.wait), m)
            ...
            client.sendList(m.chat, '', `Pilih Quality`, '', 'Klik disini ea!', sections, m)
            ...
            if (json.data.extension == 'mp3') {
               let caption = `*BOT NOLEP YTMP3 PLAYER*\n\n`
               ...
               if (chSize.oversize) return client.reply(m.chat, `💀 File size (${json.data.size}) di luar nalar, download sendiri di sini : ${await (await scrap.shorten(json.data.url)).data.url}`, m)
               ...
               let caption = `*BOT NOLEP YTMP4 PLAYER*\n\n`
               caption += `◦  *Title* : ${json.title}\n`
               caption += `◦  *Size* : ${json.data.size}\n`
               caption += `◦  *Duration* : ${json.duration}\n`
            ...
            client.sendList(m.chat, '', `Pilih Quality`, '', 'klik disini ea!', [{
               rows
            }], m)
         }
      } catch (e) {
