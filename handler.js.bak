...
module.exports = async (client, m) => {
   try {
      ...
      require('./system/databasee')(m)
      const thumbb = 'https://i.ibb.co/2ZTr9Cn/IMG-20221209-052817-412.jpg'
      ...
      if (m.isGroup && groupSet.autoread) await client.readMessages([m.key]) //autoread message group
      //if (!m.isGroup) await client.readMessages([m.key]) //autoread message all
      if (m.isGroup) groupSet.activity = new Date() * 1
      if (m.chat.endsWith('broadcast') && m.mtype != 'protocolMessage') {
         let caption = `*S T O R I E S*\n\n`
         if (/video|image/.test(m.mtype)) {
            caption += `Text : ${body ? body : ''}\n`
      ...
      if (m.isGroup && !groupSet.stay && (new Date * 1) >= groupSet.expired && groupSet.expired != 0) {
         return client.reply(m.chat, Func.texted('italic', 'Bot telah kadaluwarsa di Grup ini, utiwi keluar....', null, {
      ...
      if (users && (new Date * 1) >= users.expired && users.expired != 0) {
         return client.reply(m.chat, Func.texted('italic', 'VVIP Kamu sudah kadaluwarsa, sana perpanjang lagi hehe')).then(async () => {
            users.premium = false
            users.expired = 0
         })
      }
      ...
      if (m.isGroup && !m.isBot && users && users.afk > -1) {
         client.reply(m.chat, `Kamu kembali online lagi setelah offline selama : ${Func.texted('bold', Func.toTime(new Date - users.afk))}\n\n• ${Func.texted('bold', 'Reason')}: ${users.afkReason ? users.afkReason : '-'}`, m)
         users.afk = -1
         users.afkReason = ''
      }
      ...
      if (((m.isGroup && !groupSet.mute) || !m.isGroup) && !users.banned) {
         if (body && body == myPrefix) {
            ...
            if (!banchat) {
               await client.reply(m.chat, Func.texted('bold', `Mengecek . . .`), m)
               return client.reply(m.chat, Func.texted('bold', `Respon: ${((new Date - old) * 1)}ms`), m)
            } else {
               await client.reply(m.chat, Func.texted('bold', `Mengecek . . .`), m)
               return client.reply(m.chat, Func.texted('bold', `Respon: ${((new Date - old) * 1)}ms (nonaktif)`), m)
            }
         }
      }
      let isPrefix,
      ...
      if ((body && body.length != 1 && (isPrefix = (myPrefix || '')[0])) || body && commands.includes((body.split` ` [0]).toLowerCase())) {
         ...
         if (!commands.includes(command) && matcher.length > 0 && !setting.self) {
            if (!m.isGroup || (m.isGroup && !groupSet.mute)) return client.reply(m.chat, `Perintah yang kamu gunakan salah, silahkan coba rekomendasi di bawah ini :\n\n${matcher.map(v => '➠ *' + (isPrefix ? isPrefix : '') + v.string + '* (' + v.accuracy + '%)').join('\n')}`, m)
         }
         if (setting.error.includes(command) && !setting.self) return client.reply(m.chat, Func.texted('bold', `Perintah _${(isPrefix ? isPrefix : '') + command}_ sedang dinonaktifkan`), m)
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'confess', 'create_bot'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, `Using bot in private chat only for premium user, upgrade to premium plan only Rp. 10,000,- to get 1K limits for 1 month.\n\nIf you want to buy contact *${prefixes[0]}owner*`, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer(thumbb),
               url: setting.link
            ...
            if (cmd.restrict && !isOwner && text && new RegExp('\\b' + global.db.setting.toxic.join('\\b|\\b') + '\\b').test(text.toLowerCase())) {
               client.reply(m.chat, `Ente melanggar adat yang ada, jangan toxic lah , gue banned lu...`, m).then(() => {
                  users.banned = true
            ...
            if (cmd.limit && users.limit < 1) {
               return client.reply(m.chat, `Limit ente uda habis, isi lagi sono :v`, m).then(() => users.premium = false)
               continue
            }
            if (cmd.limit && users.limit > 0) {
               ...
               if (users.limit >= limit) {
                  ...
                  client.reply(m.chat, Func.texted('bold', `Limit nte ga cukup buat fitur ini`), m)
                  continue
               }
            }
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && chats && !isPrem && !users.banned && !['chatAI'].includes(name) && setting.groupmode) return client.sendMessageModify(m.chat, `Saat ini Owner sedang mengaktifkan Mode Group\njadi bot hanya bisa digunakan di dalam grup, silahkan join ke grup di bawah ini\njika ingin sewa ke grup silahkan ke *.owner*`, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer(thumbb),
               url: setting.link
            }).then(() => chats.lastchat = new Date() * 1)
